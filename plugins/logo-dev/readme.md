...
```bash
curl -X POST http://localhost:3456/api/plugins/logo-dev/logo_url \
...
```bash
curl -X POST http://localhost:3456/api/plugins/logo-dev/ticker_url \
  -H "Content-Type: application/json" \
  -H "X-Agent: cursor" \
  -d '{"ticker": "AAPL"}'
```
...
```bash
curl -X POST http://localhost:3456/api/plugins/logo-dev/name_url \
  -H "Content-Type: application/json" \
  -H "X-Agent: cursor" \
  -d '{"name": "Shopify"}'
```
...
```bash
curl -X POST http://localhost:3456/api/plugins/logo-dev/crypto_url \
  -H "Content-Type: application/json" \
  -H "X-Agent: cursor" \
  -d '{"symbol": "BTC"}'
```
...
- **Plugin icons**: Generate proper logos for plugins that don't have them
