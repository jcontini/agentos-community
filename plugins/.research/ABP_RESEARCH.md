# ABP (Austin Bouldering Project) Plugin Research
...
### Option 1: Generic Approach.app Plugin (Ideal)
...
### Option 2: ABP-Specific Plugin (Pragmatic)
...
**Implementation:**
1. Create `plugins/abp/` with hardcoded credentials
2. Add `instructions:` noting it's ABP-specific
3. Design entity mapping to work for any gym
4. Later: Extract to `plugins/approach/` when auth is figured out
...
auth:
  type: custom
  instructions: |
    This plugin is currently hardcoded for Austin Bouldering Project.
...
instructions: |
  This plugin is specific to Austin Bouldering Project (Springdale location).
...
1. Create `plugins/approach/` 
2. Add proper auth configuration
3. Support multiple gyms/locations
4. Deprecate ABP-specific plugin (or keep as example)
...
1. **Create plugin structure**
   ```bash
   mkdir -p plugins/abp/tests
   touch plugins/abp/readme.md
   touch plugins/abp/icon.svg
   ```
