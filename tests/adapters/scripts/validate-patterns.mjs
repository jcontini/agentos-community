 ...
 * 
 * Usage:
 *   node scripts/validate-patterns.mjs                  # Check all adapters
 *   node scripts/validate-patterns.mjs --filter reddit  # Check specific adapter
...
function validateAdapter(adapterPath, entityType, adapter) {
  const errors = [];
  const warnings = [];
  const info = [];
