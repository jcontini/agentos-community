...
Example of what a government services adapter could look like with infinite resources.

---

## Overview

A adapter for Texas Department of Motor Vehicles (TxDMV) that can:
...
# This adapter doesn't "provide" capabilities - it consumes them
# (It's not a utility other adapters would use)

# What capabilities this adapter needs from other adapters
requires:
...
# No direct auth - uses capabilities from other adapters
# (mail provider has its own auth, vision provider has its own auth)

# User profile fields this adapter uses (see user-profile.md spec)
profile:
  required:
...
instructions: |
  This adapter automates Texas DMV form submissions.
...
If validation fails, the adapter automatically adjusts (smaller font, etc.) and retries up to 3 times.
...
This adapter uses two patterns for getting work done:
...
These call other adapters that `provide` a capability:
...
- **Capabilities** are provided by adapters the user has configured
...
Instead of calling a specific adapter:
...
The adapter doesn't need to know which — it just says "sign this."
...
With this adapter, a user could say:
...
3. The adapter fills the form, validates, signs, mails
