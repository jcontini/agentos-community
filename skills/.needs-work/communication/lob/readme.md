...
Example of a "mail" capability provider that other adapters can use.

---

## Overview

Lob is a physical mail API. This adapter:
- **Provides:** `mail`, `physical-mail`, `letters`, `postcards`, `checks`
- **Enables:** Other adapters (like `tx-dot`) to mail documents without knowing Lob's specifics
...
# What capabilities this adapter provides
...
instructions: |
  Lob sends physical mail via USPS. Use this adapter to:
...
This adapter **provides** the following capabilities for other adapters/agents:
...
The `tx-dot` adapter (or any adapter needing mail) can use Lob without knowing its specifics:
...
Other adapters could also provide `mail`:
