...
curl -X POST http://localhost:3456/api/adapters/porkbun/domain.list \
  -H "Content-Type: application/json" \
  -H "X-Agent: cursor"

# List DNS records for a domain
curl -X POST http://localhost:3456/api/adapters/porkbun/domain.dns_list \
  -H "Content-Type: application/json" \
  -H "X-Agent: cursor" \
  -d '{"domain": "example.com"}'

# Add A record for GitHub Pages
curl -X POST http://localhost:3456/api/adapters/porkbun/domain.dns_create \
  -H "Content-Type: application/json" \
  -H "X-Agent: cursor" \
