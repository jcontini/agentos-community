...
2. **Write** `_projects/p<N>/<slug>/0-pain.md` — YAML frontmatter + markdown body.
3. Call `evaluate_problem(path="<absolute path to 0-pain.md>")`.
...
7. When the work is shipped, call `closeout(project="<project dir>")` to write `3-closeout.md` from the git log.

Priority `<N>` is `1`, `2`, or `3`. Ask the user if it's unclear — don't guess.

## What makes a good problem definition

You're writing `0-pain.md`, not a solution. Pretend you are a skeptical
...
hand-waving. The example at `_projects/p1/ask-user-primitive/0-pain.md`
...
- **`pass: false`**: `feedback` is a list of specific clarifying questions the evaluator wants answered. Ask the user those questions (not all at once — pick the important ones), rewrite `0-pain.md`, call `evaluate_problem` again.
...
1. A proposal agent reads `0-pain.md` and writes `1-proposal.md` — a file-level design with exact files, functions, diff sketches, edge cases, and a gate test verification plan.
2. An adversarial reviewer scores it against the pain's evaluation criteria and writes `2-review.md` with a verdict: `implement`, `revise`, or `rethink`.
...
your job after `solution` returns `implement`. Read `1-proposal.md`,
read `2-review.md`, and then go build it.
...
writes `3-closeout.md` with a five-section format: Problem, What
...
_projects/
  p1/                           <- priority 1 (active)
    <slug>/
      0-pain.md                 <- problem definition (you write, tool stamps reviews)
      1-proposal.md             <- winning proposal (solution writes)
      2-review.md               <- winning review     (solution writes)
      3-closeout.md             <- what actually shipped (closeout writes)
      _drafts/
        v1-proposal.md
        v1-review.md
